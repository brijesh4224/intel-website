...
typeof a[intel.chatConstants.VALID_PATH]!==intel.chatConstants.UNDEFINED)for(var c=a[intel.chatConstants.VALID_PATH],g=c.length,h=0;h<g;h++)if(b.includes(c[h])){d=!0;break}if(d){if(typeof a!==intel.chatConstants.UNDEFINED){currentPagePath.includes(".html")||(currentPagePath+=".html");if(null!==a.REGEX&&(b=a.REGEX,null!=b&&0<b.length))a:{var k=currentPagePath;d=b.length;for(c=0;c<d;c++){g=b[c];h=g.PATTERNS;var t=h.length;for(var u=0;u<t;u++)if((new RegExp(h[u])).test(k)){k=g[intel.chatConstants.VA_UID];
break a}}k=null}t=k}b=t;for(k=0;k<a.REGEX.length;k++)a.REGEX[k][intel.chatConstants.VA_UID]===b&&(d=a.REGEX[k][intel.chatConstants.B_UNIT]+" Virtual Assistant",chatConfigObj.virtualAssistantUid=a.REGEX[k][intel.chatConstants.VA_UID],chatConfigObj.virtualAssistantName=d,cq_tms.wa_avaamo_project="Avaamo-"+chatConfigObj.virtualAssistantName.replaceAll("Virtual Assistant","").trim());f(a)}},setMaximizationFeatureFlag:function(a){if(typeof a!==intel.chatConstants.UNDEFINED&&typeof a[intel.chatConstants.ALLOW_MAXIMIZATION]!==
...
g=intel.chatConstants.CHAT_LABELS.waitingForLiveAgent,h=intel.chatConstants.CHAT_LABELS.endChatText,k=intel.chatConstants.CHAT_LABELS.exitQueueText,t=intel.chatConstants.CHAT_LABELS.disclaimerTextPlaceholder;d.onChatBoxOpen=function(){if(chatContextObj[intel.chatConstants.APP]!==intel.chatConstants.CONTACT_SUPPORT&&(intel.chatUtility.setLocalStorageValue(intel.chatConstants.BOT_LOCAL_STORAGE_KEY,intel.chatConstants.LS_CHAT_INTERACTED,intel.chatConstants.TRUE),null===intel.chatUtility.getLocalStorageValue(intel.chatConstants.BOT_LOCAL_STORAGE_KEY,
...
function(){var n=document.getElementById("avaamo__popup");!chatConfigObj.virtualAssistantName.includes(intel.chatConstants.GCS)&&!chatConfigObj.virtualAssistantName.includes(intel.chatConstants.GAI)||intel.chatUtility.isMaximizedPage()||intel.chatUtility.detectProductContextSwitch();if(chatConfigObj.virtualAssistantName.includes(intel.chatConstants.GAI)){var q=intel.virtualAssistantChat.buildDisclaimerContent(t);n.appendChild(q)}var p=document.createElement("button");q=document.createElement("span");
p.innerHTML=intel.chatUtility.sanitizeHTML(h);p.id="live-agent-end";p.classList.add("end-live-chat");p.classList.add(intel.chatConstants.HIDE);if("true"===maximizationFeatureFlag){var l=document.createElement("button");l.innerHTML="\x3ca\x3e\x3c/a\x3e";var m=chatConfigObj.virtualAssistantName.replaceAll("Virtual Assistant","").toLowerCase().trim(),r=intel.chatConstants.MAXIMIZED_BASE_PATH+m+".html";l.addEventListener("click",function(){window.open(r,"_blank")});l.id="max_agent";n.appendChild(l);intel.chatUtility.maximizeIconVisibility();
window.addEventListener("resize",intel.chatUtility.maximizeIconVisibility)}q.innerHTML=intel.chatUtility.sanitizeHTML(c);q.id="notification-bar";q.classList.add(intel.chatConstants.HIDE);n.appendChild(p);n.appendChild(q);p.addEventListener("click",function(){p.classList.contains("end-queue")?d.sendMessage(k,"#end agent"):d.sendMessage(h,"#end agent")})};if(intel.chatUtility.isMaximizedPage()){var u=document.querySelector(".avm-red");u&&(u.className="avm-red maximised")}d.onBotMessage=function(n){var q=
...
"#FEC91B"),l=document.querySelector("#notification-bar"),m=document.querySelector("#live-agent-end");if(n.hasOwnProperty("content")){var r=document.createElement("div");r.innerHTML=n.content;r=r.querySelector("div");n.content.includes("Please give me a moment to read your chat transcript")?(l&&(l.innerHTML=intel.chatUtility.sanitizeHTML(c),l.innerHTML+=q,l.classList.remove(intel.chatConstants.HIDE)),m&&(m.innerHTML=intel.chatUtility.sanitizeHTML(h),m.classList.remove(intel.chatConstants.HIDE),m.classList.remove("end-queue"),
m.classList.contains("end-chat")||m.classList.add("end-chat"))):n.content.includes("You have already exited live agent conversation")||n.content.includes("Agent has terminated the conversation.")?intel.virtualAssistantChat.doProcessMessageOnLiveAgentChatExit(l,c,q,m):n.content.includes("Please wait, a live agent is on the way")?(l&&(l.innerHTML=intel.chatUtility.sanitizeHTML(g),l.innerHTML+=p,l.classList.remove(intel.chatConstants.HIDE)),m&&(m.innerHTML=intel.chatUtility.sanitizeHTML(k),m.classList.remove(intel.chatConstants.HIDE),
m.classList.remove("end-chat"),m.classList.contains("end-queue")||m.classList.add("end-queue"))):null!=r&&r.classList.contains("disclaimer")&&(l&&!l.classList.contains(intel.chatConstants.HIDE)&&l.classList.add(intel.chatConstants.HIDE),m&&!m.classList.contains(intel.chatConstants.HIDE)&&m.classList.add(intel.chatConstants.HIDE))}"default_card"===n.content_type&&n.attachments&&n.attachments.default_card&&(null!=n.attachments.default_card.description&&(p=document.createElement("div"),p.innerHTML=n.attachments.default_card.description,
p=p.querySelector("div"),null!=p&&p.classList.contains("disclaimer")&&(l&&!l.classList.contains(intel.chatConstants.HIDE)&&l.classList.add(intel.chatConstants.HIDE),m&&!m.classList.contains(intel.chatConstants.HIDE)&&m.classList.add(intel.chatConstants.HIDE))),n.attachments.default_card.title.includes("If you still need help you can choose one of the following ways to get help for your issue")&&intel.virtualAssistantChat.doProcessMessageOnLiveAgentChatExit(l,c,q,m))};b({message:"VA loaded",isChatInitiated:d.isChatInitiated()})})})).then(function(b){window.vaTagInjected=
...
intel.chatUtility.getLocalStorageValue(intel.chatConstants.BOT_LOCAL_STORAGE_KEY,intel.chatConstants.LS_CHAT_INTERACTED),intel.chatUtility.isNotEmpty(b)&&b===intel.chatConstants.TRUE&&768<window.innerWidth&&window.Avaamo.openChatBox(),null===intel.chatUtility.getLocalStorageValue(intel.chatConstants.BOT_LOCAL_STORAGE_KEY,intel.chatConstants.BANNER)&&(b=document.getElementById("avm-close-banner"),typeof b!==intel.chatConstants.UNDEFINED&&null!==b&&b.addEventListener("click",function(){intel.chatUtility.setLocalStorageValue(intel.chatConstants.BOT_LOCAL_STORAGE_KEY,
intel.chatConstants.BANNER,intel.chatConstants.CLOSED)})))})},doProcessMessageOnLiveAgentChatExit:function(f,a,b,d){f&&(f.innerHTML=intel.chatUtility.sanitizeHTML(a),f.innerHTML+=b,f.classList.contains(intel.chatConstants.HIDE)||f.classList.add(intel.chatConstants.HIDE));d&&!d.classList.contains(intel.chatConstants.HIDE)&&d.classList.add(intel.chatConstants.HIDE)},buildDisclaimerContent:function(f){f=intel.utility.isDataSecure(f);f.includes(intel.chatConstants.LINK_START_PLACEHOLDER)&&f.includes(intel.chatConstants.LINK_END_PLACEHOLDER)&&
(intel.chatUtility.isNotEmpty(intel.chatConstants.CHAT_LINKS.vaDisclaimerUrl)?(f=f.replace(intel.chatConstants.LINK_START_PLACEHOLDER,"\x3ca href\x3d'"+intel.chatConstants.CHAT_LINKS.vaDisclaimerUrl+"' target\x3d'_blank'\x3e"),f=f.replace(intel.chatConstants.LINK_END_PLACEHOLDER,"\x3c/a\x3e")):(f=f.replace(intel.chatConstants.LINK_START_PLACEHOLDER,intel.chatConstants.EMPTY),f=f.replace(intel.chatConstants.LINK_END_PLACEHOLDER,intel.chatConstants.EMPTY)));var a=document.createElement("div");a.innerHTML=
f;a.id="chat-disclaimer";return a}}}();"use strict";intel=intel||{};var isNewUser=!1;
...
g,null));return"user_uuid\x3d"+g}function b(c,g,h){try{if(null!=c&&typeof c!==intel.chatConstants.UNDEFINED&&c!==intel.chatConstants.EMPTY){var k=JSON.parse(c);h=h+"\x26content\x3d"+k.content+"\x26iv\x3d"+k.iv}g(h)}catch(t){}}function d(){return"https:"===location.protocol&&!0===window.isLoggedin}return{doBuildChatParameters:function(){var c=intel.chatConstants.EMPTY;c=a();var g={},h=(new URLSearchParams(window.location.search)).get(intel.chatConstants.PREFERENCE);g[intel.chatConstants.PREFERENCE]=
